...
local Framework = {}; Framework.__index = Framework; do
    function Framework:Round_V2(V2)
        return Vector2.new(mathfloor(V2.X + 0.5), mathfloor(V2.Y + 0.5))
    end
...
local ESP; ESP = {
    Settings = {
        ...
        Highlight = {Enabled = false, Color = Color3.fromRGB(1, 0, 0), Target = ""},
        ...
        Box_Outline = {Enabled = true, Color = Color3.fromRGB(0, 0, 0), Transparency = 0, Outline_Size = 1},
        Healthbar = {Enabled = false, Position = "Right", Color3.new(1, 1, 1), Color_Lerp = Color3.fromRGB(40, 252, 3)},
    ...
    function Player_Metatable:Update()
        ...
        if Character ~= nil then
            ...
            if Head and HumanoidRootPart and Current_Health > 0 then
                ...
                local Stud_Distance, Meter_Distance = mathfloor(HRP_Position.Z + 0.5), mathfloor(HRP_Position.Z / 3.5714285714 + 0.5)
                ...
                if ESP.Settings.Enabled and On_Screen and Meter_Distance < ESP.Settings.Maximal_Distance and Good then
                    ...
                    BoxFill.Transparency = Framework:Drawing_Transparency(Box_Fill_Settings.Transparency)
...
do -- // ESP Functions \\ --
    function ESP:Player(Instance, Data)
        ...
        Components.Box = Framework:Draw("Square", {Thickness = 1, ZIndex = 5})
        Components.BoxFill = Framework:Draw("Square", {Thickness = 1, ZIndex = 3})
        Components.Box_Outline = Framework:Draw("Square", {Thickness = 3, ZIndex = 4})
        Components.Healthbar = Framework:Draw("Square", {Thickness = 1, ZIndex = 5, Filled = true})
        Components.Healthbar_Outline = Framework:Draw("Square", {Thickness = 3, ZIndex = 4, Filled = true})
        Components.Name = Framework:Draw("Text", {Text = Instance.Name, Font = 2, Size = 13, ZIndex = 4, Outline = true, Center = true})
        Components.NameBold = Framework:Draw("Text", {Text = Instance.Name, Font = 2, Size = 13, Center = true})
        Components.Distance = Framework:Draw("Text", {Font = 2, Size = 13, Outline = true, ZIndex = 4, Center = true})
        Components.DistanceBold = Framework:Draw("Text", {Font = 2, Size = 13, Center = true})
        Components.Tool = Framework:Draw("Text", {Font = 2, Size = 13, Outline = true, ZIndex = 4, Center = true})
